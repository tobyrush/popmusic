...
if ( 0 < $_FILES['file']['error']) {
	...
	$songIDVars = [];
	$songIDValues = [];
	$res = $mysqli->query('DELETE FROM song_id WHERE database_name="rs200"');
	while (($data = fgetcsv($handle, 0, "\t")) !== FALSE) {
		...
		if ($res->num_rows) {
			...
			$res = $mysqli->query('INSERT INTO song_id (identifier, song, database_name) VALUES ("' . $data[0] . '",' . $songID . ',"rs200")');
		} else {
